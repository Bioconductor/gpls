...
@
{\tt K.prov} specifies the number PLS components to use. Note that when {\tt K.prov} is no specified, the number of PLS components are set to be the smaller of the row and column rank of the design matrix.\\

\section*{The {\tt glpls1a.cv.error} and {\tt glpls1a.train.test.error} function}
The {\tt glpls1a.cv.error} calculates leave-one-out classification error rate for two-group classification and {\tt glpls1a.train.test.error} calculates test set error where the model is fit using the training set.\\
...
@ 
\section*{The {\tt glpls1a.mlogit} and {\tt glpls1a.logit.all} functions}
The {\tt glpls1a.mlogit} carries out multi-group classification using MIRWPLS(F) where the baseline logit model is used as counterpart to {\tt glpls1a} for two group case. {\tt glpls1a.logit.all} carries out multi-group classification by separately fitting $C$ two-group classification using {\tt glpls1a) separately for $C$ group vs the same baseline class (i.e. altogether $C+1$ classes). This separate fitting of logit is known to be less efficient but has been used in practice due to its more straightforward implementation.\\

Note that when using {\tt glpls1a.mlogit}, the X matrix needs to have a column of one, i.e. intercept term.
...
@ 
\section*{The {\tt glpls1a.mlogit.cv.error} function}
The {\tt glpls1a.mlogit.cv.error} calculates leave-one-out error for multi-group classification using (M)IRWPLS(F). When the {\tt mlogit} option is set to be true, then {\tt glpls1a.mlogit} is used, else {\tt glpls1a.logit.all} is used for fitting.\\
